...
public class HallOfFameController : ControllerBase
{
    ...
    // playerId
    [HttpGet("{playerId}", Name = "GetHallOfFameByPlayerId")]
    public async Task<ActionResult<List<HallOfFame>>> Get(string playerId)
    {
        try
        {
            return Ok(await _supervisor.GetHallOfFameByPlayerId(playerId));
        }
        catch (Exception ex)
        {
            _logger.LogError($"Something went wrong inside the HallOfFameController GetByPlayerId action: {ex}");
            return StatusCode(500, "Internal server error");
        }
    }
}