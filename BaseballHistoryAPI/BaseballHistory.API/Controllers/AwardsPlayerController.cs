...
public class AwardsPlayerController : ControllerBase
{
    ...
    // playerId
    [HttpGet("{playerId}", Name = "GetAwardsPlayerByPlayerId")]
    public async Task<ActionResult<List<AwardsPlayer>>> Get(string playerId)
    {
        try
        {
            return Ok(await _supervisor.GetAwardsPlayerByPlayerId(playerId));
        }
        catch (Exception ex)
        {
            _logger.LogError($"Something went wrong inside the AwardsPlayerController GetByPlayerId action: {ex}");
            return StatusCode(500, "Internal server error");
        }
    }
}